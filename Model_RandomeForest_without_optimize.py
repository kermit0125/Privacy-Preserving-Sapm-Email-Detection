...
# 创建结果文件夹
output_dir = './RandomeForest_model_results_original/'
...
# 4. 特征重要性分析
# 使用原始单词作为特征名称
feature_importances = pd.Series(model.feature_importances_, index=original_feature_names)

# 可视化前10个重要特征
plt.figure(figsize=(12, 6))
feature_importances.nlargest(10).plot(kind='bar', color='skyblue')
plt.title('Top 10 Feature Importances (Original Data)')
...
# 保存模型
joblib.dump(model, os.path.join(output_dir, 'random_forest_model_original.pkl'))

print(f"随机森林模型已训练完成（原始数据），所有结果保存到文件夹：{output_dir}")
